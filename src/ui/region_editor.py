...
REGION_DEFINITIONS: list[dict[str, str]] = [
    {"key": "playing_left", "label": "上家出牌区域", "usage": "框住上家打出的牌会出现的位置。", "example": "通常在桌面左侧中间。"},
    {"key": "playing_middle", "label": "我的出牌区域", "usage": "框住自己打出的牌会出现的位置。", "example": "通常在桌面中间偏下。"},
    {"key": "playing_right", "label": "下家出牌区域", "usage": "框住下家打出的牌会出现的位置。", "example": "通常在桌面右侧中间。"},
    {"key": "my_cards", "label": "我的手牌区域", "usage": "框住自己当前手里的整排牌。", "example": "通常在屏幕底部。"},
    {"key": "avatar_left", "label": "上家头像区域", "usage": "框住上家头像和地主标志位置。", "example": "通常在左侧玩家头像附近。"},
    {"key": "avatar_middle", "label": "我的头像区域", "usage": "框住自己的头像和地主标志位置。", "example": "通常在底部中间头像附近。"},
    {"key": "avatar_right", "label": "下家头像区域", "usage": "框住下家头像和地主标志位置。", "example": "通常在右侧玩家头像附近。"},
    {"key": "game_over", "label": "游戏结束标志区域", "usage": "框住结算或结束标志出现的位置，用于判断本局结束。", "example": "通常在结算弹层或中间提示区域。"},
]
...
class SettingsPanel(ttk.Frame):
    ...
    def _build_variables(self) -> None:
        ...
        self.threshold_vars = {
            ...
            "gameover": tk.DoubleVar(value=0.9),
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(footer, text="所有参数都可以在这里修改，保存后立即生效。").grid(row=0, column=0, sticky="w")
    ...
    def _build_region_tab(self) -> None:
        ...
        ttk.Label(toolbar, text="点中已有框可直接拖动，点框外拖拽会重画矩形。").grid(row=0, column=0, sticky="w")
        ...
        ttk.Entry(origin_frame, textvariable=self.game_origin_vars["y"]).grid(row=0, column=3, sticky="ew")
        ttk.Button(origin_frame, text="按输入值整体平移所有区域", command=self._apply_game_origin_entries).grid(row=1, column=0, columnspan=4, sticky="ew", pady=(8, 0))
        ...
        ttk.Label(info_frame, textvariable=self.region_name_var, font=("Microsoft YaHei UI", 10, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Label(info_frame, textvariable=self.region_usage_var, wraplength=360, justify="left").grid(row=1, column=0, sticky="w", pady=(8, 0))
        ttk.Label(info_frame, textvariable=self.region_example_var, wraplength=360, justify="left", foreground="#5c6b80").grid(row=2, column=0, sticky="w", pady=(6, 0))
        ...
        for field, row, column in (("x1", 0, 0), ("y1", 0, 2), ("x2", 1, 0), ("y2", 1, 2)):
            ttk.Label(coord_frame, text=field.upper()).grid(row=row, column=column, sticky="w", pady=4)
            ttk.Entry(coord_frame, textvariable=self.coord_vars[field]).grid(row=row, column=column + 1, sticky="ew", padx=(6, 12), pady=4)
        ttk.Button(coord_frame, text="应用当前区域坐标", command=self._apply_region_entries).grid(row=2, column=0, columnspan=4, sticky="ew", pady=(8, 0))
    ...
    def _build_window_tab(self) -> None:
        ...
        for key, label in WINDOW_LABELS.items():
            ...
            row = 0
            if key != "SWITCH":
                ttk.Checkbutton(tab, text="显示这个窗口", variable=self.gui_vars[key]["DISPLAY"]).grid(row=row, column=0, columnspan=2, sticky="w", pady=4)
                row += 1
            for field, title in (("OPACITY", "透明度"), ("FONT_SIZE", "字号"), ("OFFSET_X", "左上角 X"), ("OFFSET_Y", "左上角 Y"), ("CENTER_X", "中心点 X"), ("CENTER_Y", "中心点 Y")):
    ...
    def _build_other_tab(self) -> None:
        ...
        for idx, (key, label) in enumerate((("QUIT", "退出程序"), ("OPEN_LOG", "打开日志"), ("OPEN_SETTINGS", "切到设置页"), ("RESET", "重置记牌"))):
        ...
        ttk.Combobox(log_frame, textvariable=self.log_vars["level"], values=["TRACE", "DEBUG", "INFO", "SUCCESS", "WARNING", "ERROR", "CRITICAL"], state="readonly").grid(row=0, column=1, sticky="ew", padx=(8, 0), pady=4)
    ...
    def load_from_config(self) -> None:
        ...
        self._regions = {item["key"]: [point[:] for point in config_model.REGIONS[item["key"]]] for item in self._definitions}
        self._game_origin = (int(config_model.GAME_WINDOW["OFFSET_X"]), int(config_model.GAME_WINDOW["OFFSET_Y"]))
        self.game_origin_vars["x"].set(self._game_origin[0])
        self.game_origin_vars["y"].set(self._game_origin[1])
        for key in ("card", "landlord", "pass", "wait", "gameover"):
    ...
    def save_all(self) -> None:
        try:
            ...
            return
        for item in self._definitions:
            (x1, y1), (x2, y2) = config["REGIONS"][item["key"]]
            if x2 <= x1 or y2 <= y1:
    ...
    def _apply_region_entries(self) -> None:
        self._set_region_from_points(self.coord_vars["x1"].get(), self.coord_vars["y1"].get(), self.coord_vars["x2"].get(), self.coord_vars["y2"].get())
